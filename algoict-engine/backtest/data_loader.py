...
def load_data_csv(path) -> pd.DataFrame:
    """
    Load a simple OHLCV CSV (non-Databento).

    Expected columns: timestamp, open, high, low, close, volume (or aliases)
    Returns: DataFrame with DatetimeIndex in America/Chicago.
    """
    path = Path(path)
    df = pd.read_csv(path)

    # Rename aliases to standard names
    col_map = {
        "time": "timestamp",
        "datetime": "timestamp",
        "date": "timestamp",
...
def _build_continuous(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    For each 1-min timestamp, keep the row belonging to the contract
    with the highest cumulative daily volume.
    """
